...
public class CSDAO implements CSService {
	...
	public CSDTO csSelectDetail(int cs_number) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				csDTO.setMember_number(resultSet.getInt("member_number"));
			}
		} catch (Exception e) {
	...
	public CSDTO csInsert(CSDTO csDTO) {
		...
		try {
			...
			DataSource dataSource = (DataSource) context.lookup("java:comp/env/jdbc");
			connection = dataSource.getConnection();
			String sql = "insert into ci (cs_number, cs_title, cs_date, cs_content, member_number)";
...
//			preparedStatement.setInt(1, csDTO.getCs_number()); cf) 시퀀스 적용
			...
			preparedStatement.setInt(4, csDTO.getMember_number());
	...
	public ArrayList<CSDTO> csSelectMine(int member_number){
		...
		try {
			...
			DataSource dataSource = (DataSource) context.lookup("java:comp/env/jdbc");
			connection = dataSource.getConnection();
			String sql = "select * from ci where member_number = ? order by cs_number desc";
			log.info("SQL 확인 - " + sql);
